...
def momo_executable():
    ...
    if not target:
        ...
        if system == "darwin":
            ...
            pytest.skip(f"Cannot auto-detect target for {system}/{machine}. Please set MOMO_TARGET environment variable.")
    ...
    if not momo_path.exists():
        pytest.skip(f"momo executable not found at {momo_path}. Please build with: python3 run.py build {target}")
...
def momo_server(momo_executable):
    ...
    with httpx.Client() as client:
        for _ in range(30):  # 最大30秒待機
            ...
            pytest.fail(f"momo server failed to start within 30 seconds\nstdout: {stdout.decode('utf-8', errors='ignore')}\nstderr: {stderr.decode('utf-8', errors='ignore')}")
...
def sora_server(momo_executable):
    ...
    # 環境変数から設定を取得
    signaling_urls = os.environ.get("TEST_SORA_SIGNALING_URLS")
    channel_id_prefix = os.environ.get("TEST_SORA_CHANNEL_ID_PREFIX", "")
    
    if not signaling_urls:
        pytest.skip("TEST_SORA_SIGNALING_URLS is not set in .env file")
    
    # チャンネル ID を自動生成（プレフィックス + UUID）
    import uuid
    ...
    cmd = [
        ...
        "sora",
        "--signaling-urls", signaling_urls,
        "--channel-id", channel_id,
        "--role", "sendonly",  # テスト用にsendonly
        "--audio", "true",     # オーディオを有効化
        "--video", "true",     # ビデオを有効化
    ]
    
    # オプションのメタデータを追加
    metadata = os.environ.get("TEST_SORA_METADATA")
    if metadata:
        cmd.extend(["--metadata", metadata])
    
    # オプションのクライアントIDを追加
    client_id = os.environ.get("TEST_SORA_CLIENT_ID")
    if client_id:
        cmd.extend(["--client-id", client_id])
    
    # オプションのバンドルIDを追加
    bundle_id = os.environ.get("TEST_SORA_BUNDLE_ID")
    if bundle_id:
    ...
    with httpx.Client() as client:
        for _ in range(30):  # 最大30秒待機
            ...
            process.terminate()
            process.wait(timeout=5)
            pytest.fail(f"sora server failed to start within 30 seconds\nstdout: {stdout.decode('utf-8', errors='ignore')}\nstderr: {stderr.decode('utf-8', errors='ignore')}")

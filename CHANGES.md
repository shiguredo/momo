...
- [ADD] `--force-yuy2` オプションを追加する
  - YUY2 フォーマットを強制的に使用し、サポートされていない場合はエラーで終了する
  - Intel VPL での H.265 エンコード時の高フレームレート実現に有用
  - @voluntas
- [CHANGE] `--framerate` オプションの上限を 60 から 240 に変更する
  - 高フレームレートカメラ（120fps、240fps）のサポートを可能にする
  - @voluntas
- [FIX] `--force-yuy2` 使用時にカメラのケーパビリティを適切に選択するよう修正
  - デバイスがサポートする全ケーパビリティを列挙して最適なものを選択
  - YUY2 フォーマットで指定された解像度・フレームレートをサポートするケーパビリティを探す
  - Intel VPL エンコーダーのフレームレート設定ログを追加
  - @voluntas

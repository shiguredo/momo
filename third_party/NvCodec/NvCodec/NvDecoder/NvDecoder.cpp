...
int NvDecoder::HandleVideoSequence(CUVIDEOFORMAT* pVideoFormat) {
  ...
  if (m_eChromaFormat == cudaVideoChromaFormat_420 ||
    ...
    m_eOutputFormat =
        cudaVideoSurfaceFormat_NV12;  // no 4:2:2 output format supported yet so make 420 default
...
int NvDecoder::HandlePictureDisplay(CUVIDPARSERDISPINFO* pDispInfo) {
    ...
    if (m_SEIMessagesDisplayOrder[pDispInfo->picture_index].pSEIData) {
      ...
      if (m_fpSEI) {
        for (uint32_t i = 0; i < seiNumMessages; i++) {
          if (m_eCodec == cudaVideoCodec_H264 || cudaVideoCodec_H264_SVC ||
              cudaVideoCodec_H264_MVC || cudaVideoCodec_HEVC) {
  ...
  CUDA_DRVAPI_CALL(dyn::cuCtxPushCurrent(m_cuContext));
  NVDEC_API_CALL(dyn::cuvidMapVideoFrame(m_hDecoder, pDispInfo->picture_index,
    ...
    if ((unsigned)++m_nDecodedFrame > m_vpFrame.size()) {
      ...
      if (m_bUseDeviceFrame) {
        if (m_bDeviceFramePitched) {
          CUDA_DRVAPI_CALL(dyn::cuMemAllocPitch(
  ...
  NVDEC_API_CALL(dyn::cuvidUnmapVideoFrame(m_hDecoder, dpSrcFrame));
  return 1;
}
...
int NvDecoder::Decode(const uint8_t* pData,
                      int nSize,
                      int nFlags,
                      int64_t nTimestamp) {
  m_nDecodedFrame = 0;
  ...
  NVDEC_API_CALL(dyn::cuvidParseVideoData(m_hParser, &packet));

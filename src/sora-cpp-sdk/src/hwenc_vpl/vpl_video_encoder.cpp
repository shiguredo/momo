...
int32_t VplVideoEncoderImpl::Encode(
  ...
  if (video_frame_buffer->type() == webrtc::VideoFrameBuffer::Type::kNative) {
    ...
    if (native_buffer->VideoType() == webrtc::VideoType::kYUY2 && !use_vpp_) {
      ...
      if (!use_vpp_) {
        RTC_LOG(LS_INFO)
            << "VPP initialization failed, will use software conversion";
      }
    }
  }
  ...
  if (use_vpp_ &&
    ...
    auto native_buffer = static_cast<NativeBuffer*>(video_frame_buffer.get());
    RTC_LOG(LS_VERBOSE) << "VPP path: Native buffer type = "
                        << (int)native_buffer->VideoType();
    if (native_buffer->VideoType() == webrtc::VideoType::kYUY2) {
      ...
      mfxStatus sts = vpp_->RunFrameVPPAsync(
          &*vpp_input_surface, &*vpp_output_surface, nullptr, &vpp_syncp);
      ...
      // VPP の処理を待つ
      sts =
          MFXVideoCORE_SyncOperation(GetVplSession(session_), vpp_syncp, 60000);
      VPL_CHECK_RESULT(sts, MFX_ERR_NONE, sts);
      ...
      RTC_LOG(LS_VERBOSE)
          << "VPP converted YUY2 to NV12 using GPU acceleration";
    } else {
      ...
      libyuv::I420ToNV12(frame_buffer->DataY(), frame_buffer->StrideY(),
                         ...
                         surface->Data.Pitch, frame_buffer->width(),
                         frame_buffer->height());
    }
  } else {
    ...
    if (video_frame_buffer->type() == webrtc::VideoFrameBuffer::Type::kNative) {
      ...
      if (native_buffer->VideoType() == webrtc::VideoType::kYUY2) {
        ...
        RTC_LOG(LS_INFO)
            << "Using software YUY2 to NV12 conversion (VPP not available)";
        libyuv::YUY2ToNV12(native_buffer->Data(), native_buffer->RawWidth() * 2,
                           surface->Data.Y, surface->Data.Pitch,
                           surface->Data.U, surface->Data.Pitch,
                           native_buffer->RawWidth(),
                           native_buffer->RawHeight());
      } else {
...
int32_t VplVideoEncoderImpl::InitVpp() {
  ...
  if (sts < MFX_ERR_NONE) {
    RTC_LOG(LS_WARNING)
        << "VPP Query failed, falling back to software conversion: " << sts;
    vpp_.reset();
    return WEBRTC_VIDEO_CODEC_OK;
  }
  ...
  if (sts != MFX_ERR_NONE) {
    RTC_LOG(LS_WARNING)
        << "VPP Init failed, falling back to software conversion: " << sts;
    vpp_.reset();
    return WEBRTC_VIDEO_CODEC_OK;
  }
  ...
  RTC_LOG(LS_INFO) << "VPP Input NumFrameSuggested="
                   << vpp_alloc_request_[0].NumFrameSuggested
                   << " Output NumFrameSuggested="
                   << vpp_alloc_request_[1].NumFrameSuggested;
  ...
  // VPP 入力サーフェース（YUY2）の作成
  {
    ...
    vpp_input_surface_buffer_.resize(vpp_alloc_request_[0].NumFrameSuggested *
                                     size);
  ...
  // VPP 出力サーフェース（NV12）の作成
  {
    ...
    int size = width * height * 12 / 8;
    vpp_output_surface_buffer_.resize(vpp_alloc_request_[1].NumFrameSuggested *
                                      size);

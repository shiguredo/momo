...
2. 仮に `diff-sora-cpp-sdk.sh` の結果の方が小さい場合:
    1. Momo の `src/sora-cpp-sdk` に移動して `./diff-sora-cpp-sdk.sh ../../../sora-cpp-sdk <target_commit> | patch -p1` でパッチを適用する
    2. コンフリクトが起きたら修正する
    3. `./copy-to-sora-cpp-sdk.sh ../../../sora-cpp-sdk` を実行して Sora C++ SDK にもソースを反映する
3. 仮に `diff-momo.sh` の結果の方が小さい場合:
    1. Sora C++ SDK に移動して `../momo/sora-cpp-sdk/diff-sora-cpp-sdk.sh <sora-cpp-sdk dir> <target_commit> | patch -p1` でパッチを適用する
    2. コンフリクトが起きたら修正する

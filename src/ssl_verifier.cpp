...
bool SSLVerifier::VerifyHost(const std::string& hostname,
  ...
  auto endpoints = resolver.resolve(hostname, port, error);
  if (error) {
    return false;
  }
  ...
  // cert 一覧の設定
  {
    bool result = rtc::openssl::LoadBuiltinSSLRootCertificates(ctx);
    if (!result) {
      SSL_CTX_free(ctx);
      RTC_LOG(LS_ERROR) << "LoadBuiltinSSLRootCertificates failed";
      return false;
    }
  ...
  if (ssl_error != SSL_ERROR_NONE) {
    SSL_free(ssl);
    SSL_CTX_free(ctx);
    RTC_LOG(LS_ERROR) << "SSL Connection error code=" << code
                      << " error=" << ssl_error;
    return false;
  }
  ...
  if (cert == nullptr) {
    SSL_free(ssl);
    SSL_CTX_free(ctx);
    RTC_LOG(LS_ERROR) << "SSL_get_peer_certificate failed";
    return false;
  }

  bool is_valid_cert_name =
      X509_check_host(cert, hostname.c_str(), hostname.size(), 0, nullptr) == 1;
  if (!is_valid_cert_name) {
    X509_free(cert);
    SSL_free(ssl);
    SSL_CTX_free(ctx);
    RTC_LOG(LS_ERROR) << "X509_check_host failed";
    return false;
  }
  X509_free(cert);

  bool verify_result = SSL_get_verify_result(ssl) == X509_V_OK;
  if (!verify_result) {
    SSL_free(ssl);
    SSL_CTX_free(ctx);
    RTC_LOG(LS_ERROR) << "SSL_get_verify_result failed";
    return false;
  }

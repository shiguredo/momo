...
webrtc::PeerConnectionInterface::IceConnectionState SoraWebsocketClient::getRTCConnectionState() const { return rtc_state_; }
...
SoraWebsocketClient::SoraWebsocketClient(boost::asio::io_context& ioc, RTCManager* manager, ConnectionSettings conn_settings)
    : ioc_(ioc)
    , resolver_(ioc)
    , manager_(manager)
    , retry_count_(0)
...
void SoraWebsocketClient::onResolve(boost::system::error_code ec, boost::asio::ip::tcp::resolver::results_type results) {
    if (ec) {
        reconnectAfter();
    ...
    if (ws_->isSSL()) {
        boost::asio::async_connect(
            ...
            boost::asio::bind_executor(
                ws_->strand(),
                std::bind(
                    &SoraWebsocketClient::onSSLConnect,
                    shared_from_this(),
                    std::placeholders::_1)));
    } else {
        boost::asio::async_connect(
...
void SoraWebsocketClient::onSSLConnect(boost::system::error_code ec) {
    ...
    ws_->nativeSecureSocket().next_layer().async_handshake(
        ...
        boost::asio::bind_executor(
            ws_->strand(),
            std::bind(
                &SoraWebsocketClient::onSSLHandshake,
                shared_from_this(),
                std::placeholders::_1)));
}
...
void SoraWebsocketClient::onSSLHandshake(boost::system::error_code ec) {
    ...
    ws_->nativeSecureSocket().async_handshake(parts_.host, parts_.path_query_fragment,
        boost::asio::bind_executor(
            ws_->strand(),
            std::bind(
                &SoraWebsocketClient::onHandshake,
                shared_from_this(),
                std::placeholders::_1)));
}
...
void SoraWebsocketClient::onRead(boost::system::error_code ec, std::size_t bytes_transferred, std::string text)
{
...
void SoraWebsocketClient::onIceConnectionStateChange(webrtc::PeerConnectionInterface::IceConnectionState new_state) {
    RTC_LOG(LS_INFO) << __FUNCTION__ << " state:" << new_state;
    boost::asio::post(
        ws_->strand(),
        std::bind(
            &SoraWebsocketClient::doIceConnectionStateChange,
            shared_from_this(),
            new_state));
}
void SoraWebsocketClient::onIceCandidate(const std::string sdp_mid, const int sdp_mlineindex, const std::string sdp) {
    json json_message = {
      {"type", "candidate"},
      {"candidate", sdp}
    };
...
void SoraWebsocketClient::onCreateDescription(webrtc::SdpType type, const std::string sdp) {
    json json_message = {
      {"type", "answer"},
      {"sdp", sdp}
    };

...
private:
    static boost::beast::http::response<boost::beast::http::string_body> createOKwithJson(const boost::beast::http::request<boost::beast::http::string_body>& req, nlohmann::json json_message);

    template<class Body, class Fields>
    void sendResponse(boost::beast::http::response<Body, Fields> msg) {
        auto sp = std::make_shared<boost::beast::http::response<Body, Fields>>(std::move(msg));

...
class AyameWebsocketClient : public std::enable_shared_from_this<AyameWebsocketClient>, public RTCMessageSender
{
...
public:
    AyameWebsocketClient(boost::asio::io_context& ioc, RTCManager* manager, ConnectionSettings conn_settings);
    void reset();
...
private:
    ...
    void onIceConnectionStateChange(webrtc::PeerConnectionInterface::IceConnectionState new_state) override;
    void onIceCandidate(const std::string sdp_mid, const int sdp_mlineindex, const std::string sdp) override;
...
private:
    void doIceConnectionStateChange(webrtc::PeerConnectionInterface::IceConnectionState new_state);
};

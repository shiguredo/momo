...
///
/// C++ 版の SetCodecPreferences と同等の処理:
/// 1. PeerConnectionFactory から送受信両方の capabilities を取得
/// 2. 共通コーデックを抽出
/// 3. 指定コーデックを primary、補助コーデックを secondary としてフィルタリング
/// 4. Transceiver に SetCodecPreferences を呼ぶ
fn set_codec_preferences(
    ...
    if let (Some(target_codec), Some(transceiver)) = (video_codec_type, video_transceiver) {
        ...
        transceiver
            ...
            .map_err(wrtc_err)?;
        info!(target: "ayame", codec = target_codec, "video codec preferences set");
    }
    ...
    if let Some(target_codec) = audio_codec_type {
        ...
        audio_transceiver
            .set_codec_preferences(&filtered)
            .map_err(wrtc_err)?;
        info!(target: "ayame", codec = target_codec, "audio codec preferences set");
    }
...
fn filter_codecs(
    ...
    // sender 側の各コーデックについて、receiver 側に同じ mime_type があれば共通とみなす

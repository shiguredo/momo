...
Momo は **Intel VPL 限定** で H.265 (HEVC) エンコードにおいて YUY2 フォーマットの直接入力をサポートしています。これにより、YUY2 出力対応のカメラから取得した映像データを変換なしで直接エンコードでき、高フレームレート（最大 240fps）の実現が可能になります。

**重要:** YUY2 サポートは Intel VPL エンコーダー限定の機能です。他のエンコーダー（x264、OpenH264、VP9、AV1 など）では利用できません。
...
- Ubuntu 24.04 x86_64
- YUY2 出力対応のカメラデバイス
- H.265 エンコーダーとして Intel VPL を使用（`--h265-encoder vpl`）
...
Intel VPL では HEVC (H.265) の Range Extension プロファイルを使用して YUV422 入力をサポートしています。これにより、カメラから取得した YUY2 データをフォーマット変換なしで直接エンコードできます。

**処理フローの最適化：**

- 従来：カメラ (YUY2) → I420 変換 → NV12 変換 → H.265 エンコード  
- 最適化後：カメラ (YUY2) → H.265 エンコード（直接）

この最適化により、CPU 使用率の削減とレイテンシの短縮が実現できます。
...
1 から 240 までのフレームレートを指定できます（デフォルト: 30）。
...
- **Intel VPL エンコーダー限定**（他のエンコーダーでは YUY2 サポートなし）
- H.265 コーデック限定（H.264、VP9、AV1 では利用不可）
- Linux 環境限定（Ubuntu 24.04 推奨）
- カメラデバイスが YUY2 出力に対応している必要がある
- --force-yuy2 使用時は YUY2 が利用できない場合エラー終了（フォールバックなし）

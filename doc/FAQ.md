...
はい。 Momo のライセンスは [Apache License, Version 2\.0](http://www.apache.org/licenses/LICENSE-2.0) で公開しており、商用利用もできます。

## コーデックの指定はできますか？

はい。コーデックの指定は Ayame モードと Sora モードで利用できます。

## ビットレートは指定できますか？

はい。ビットレートの指定は Sora モードでのみ利用できます。

## サイマルキャストは利用できますか？

はい。Sora モードでのみ利用できます。
...
はい。 P2P モードと Ayame モードでシリアル経由でのみ、データチャネルが利用できます。

[USE_SERIAL.md](USE_SERIAL.md)

## 認証局の証明書を追加できますか？

はい。 `SSL_CERT_DIR` または `SSL_CERT_FILE` 環境変数に CA 証明書のパスを指定することで、サーバ証明書の検証に利用するための CA 証明書を追加することができます。
...
はい。 `--insecure` オプションを指定することで、クライアント側でサーバ証明書の検証を行わないようにすることができます。
...
いいえ。 Momo では Sora の DataChannel メッセージングに対応する予定はありません。 Sora C++ SDK にて対応予定です。
...
また、プロファイルレベル ID を変更することができない P2P モードや Ayame モードでは FHD 配信ができません。

こちらも H.264 の HD 以下で配信するか、他のコーデックを使用して FHD 配信をしてください。

## Windows で H.264 を利用できますか？

はい。 Windows では NVIDIA Video Codec、または Intel VPL のハードウェアアクセラレーターで H.264 を利用できます。
...
いいえ。 Momo は Sora の TURN 機能を無効にして利用することはできません。

## Sora モードでマルチストリーム機能を無効にすることはできますか？

いいえ。 Momo の Sora モードではマルチストリーム機能を無効にして利用することはできません。

## Proxy を設定して利用することはできますか？

はい。以下に示すオプションを指定することで利用できます。
...
はい。 `--resolution` オプションを使用することで解像度の設定ができます。
...
JetPack 6.0.0 以外のバージョンでは利用できません。

...
はい。 `SSL_CERT_DIR` または `SSL_CERT_FILE` 環境変数に CA 証明書のパスを指定することで、サーバー証明書の検証に利用するための CA 証明書を追加できます。
...
はい。 `--insecure` オプションを指定することで、クライアント側でサーバー証明書の検証を行わないようにできます。
...
また、プロファイルレベル ID を変更できない P2P モードや Ayame モードでは FHD 配信ができません。

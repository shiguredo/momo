...
- Raspberry Pi 4 / Bookworm で `./momo --no-audio-device --log-level 0 --hw-mjpeg-decoder true --resolution FHD p2p` を実行し、別の PC の Chrome で `http://<Raspberry Pi 4 の IP アドレス>:8080/html/p2p.html` を開いて接続すると、受信側の映像に緑の線が入る (p2p モードの確認手順は `doc/USE_P2P.md` 参照)
...
- momo 2023.1.0 および 2025.1.0-canary.9 で再現した (どちらも当時のコマンドは `test` モード。2025.1.0 で `p2p` モードへ名前変更されたため、現行 develop では `p2p` を使う)
...
- まず現行 develop で再現することを確認する (Raspberry Pi 4 + USB カメラ + `--hw-mjpeg-decoder true --resolution FHD`、p2p モード)
...
- 修正後は Raspberry Pi 4 + USB カメラ + `--hw-mjpeg-decoder true` で FHD / HD の H.264 を配信し、p2p モードの受信側で緑の線が出ないことを確認する

## 完了条件

- Raspberry Pi 4 + USB カメラで `--hw-mjpeg-decoder true --resolution FHD` の H.264 を p2p モードで配信し、Chrome で受信したとき、映像に緑の線が入らない
